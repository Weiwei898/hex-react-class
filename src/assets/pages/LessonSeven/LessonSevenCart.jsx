...
// 因應LessonSix作業進行LessonSevenCart最小改動，(引入自定義 Hook)
...
export default function LessonSevenCart() {
  // 因應LessonSix作業進行LessonSevenCart最小改動，(驗證登入狀態，useNavigate 進行頁面跳轉)
  const navigate = useNavigate();
  const [cart, setCart] = useState({});
  // 使用 useDispatch 取得 dispatch 函式，用於發送 Redux Action
  ...
  // 因應LessonSix作業進行LessonSevenCart最小改動，(使用 useCart Hook 取出需要的資料與函式，並傳入 getCart 以便更新畫面)
  ...
  const clearCart = async () => {
    try {
      ...
      dispatch(pushMessage({ text: '購物車已清空', status: 'success' }));
      getCart();
    } catch (error) {
      console.error(error);
      dispatch(pushMessage({ text: '清空購物車失敗', status: 'danger' }));
    }
  };
  ...
  return (
    <div className="container mt-4 lesson-seven">
      <h2 className="mb-4 fw-bold text-white">購物車</h2>
      ...
      {cart.carts?.length > 0 ? (
        <div className="card border-0 shadow-sm overflow-hidden">
          ...
          <div className="card-footer bg-dark border-top border-secondary p-3 d-flex justify-content-end">
            ...
            <button type="button" className="btn btn-outline-danger me-2" onClick={clearCart}>
              清空購物車
            </button>
